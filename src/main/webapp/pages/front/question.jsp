	...
	<div class="item " itemIndex="${status.count}">
	                    ...
	                    <c:forEach var="answer" items="${question.getAnswerList()}" begin="0" end ="${question.getAnswerList().size()}" varStatus="anstatus">
		                    ...
		                    <span style="font-size:1.2em;color:#333"> <em><c:out value="${anstatus.count}"/>.</em>${answer.ansContent}</span>
		                        <input type="radio" id="singleRadio_${anstatus.count}" style="float:right;" onclick="QuestionApi.nextQuestion(${good.id},this,'${answer.nextQuestionId}','${answer.nextQuesNum}')"  value="option_${anstatus.count}" name="radioSingle_${anstatus.count}" aria-label="Single radio One">
		                        <div>
